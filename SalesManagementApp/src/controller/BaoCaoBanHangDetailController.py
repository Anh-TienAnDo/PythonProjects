...
class BaoCaoBanHangDetailController:
    def __init__(self, parent: Frame, id_mat_hang: str, month: str, year: str):
        ...
        self.column_title = COLUMNS_REPORT_BAN_HANG_DETAIL
    ...
    def report(self):
        ...
        try:
            ...
            report_ban_hang_list = self.ban_hang_service.report_detail_mat_hang(sort=sort, month=month, year=year, id_mat_hang=self.id_mat_hang)
    ...
    def fo_back(self):
        ...
        try:
            self.frame.destroy()
            BaoCaoBanHangController(self.parent)
        except (ConnectionError, TimeoutError, ValueError) as e:
            logging.error("Error: %s", e)
    ...
    def refresh_ban_hang_list(self):
        ...
        report_ban_hang_list = self.report()
        # add title for table
        self.show_column_title()
        # Thêm các Label và Button vào scrollable_frame
        row = 1
        total_ban_hang_temp = 0
        total_so_luong_temp = 0
        total_thanh_tien_temp = 0
        for ban_hang in report_ban_hang_list:
            total_ban_hang_temp += int(ban_hang[2])
            total_so_luong_temp += int(ban_hang[3])
            total_thanh_tien_temp += int(ban_hang[4])
    ...
    def init_components(self):
        ...
        head_label = LabelType.h1(self.head_frame, f"{TITLE_REPORT_BAN_HANG_DETAIL}: {self.id_mat_hang} - {self.date.get('month')}/{self.date.get('year')}") # Label trong head_frame
        ...
        sort_combobox['values'] = self.ban_hang_service.get_report_ban_hang_detail_sort_keys()
        ...
        # total
        total_ban_hang_label = LabelType.normal_blue_white(self.head_frame, text="Tổng lần bán:")
        total_ban_hang_label.grid(row=1, column=2, sticky="e", padx=5, pady=5)
        total_ban_hang_value = EntryType.view(self.head_frame, text_var=self.total_ban_hang)
        total_ban_hang_value.grid(row=1, column=3, sticky="w", padx=5, pady=5)
